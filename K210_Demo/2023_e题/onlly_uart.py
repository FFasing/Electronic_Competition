...
def find_point():
    ...
    while mod==1:
        ...
        img=img.to_rainbow()
        rainbow_point = img.find_blobs(Point_Rainbow_threshold,x_stride=1,y_stride=1,pixels_threshold=10,area_threshold=1)
        if rainbow_point:
...
rect_solw=0
def fun(tim):
    global key_set_slow_down,rect_solw,uart_flag
    key_set_slow_down=key_set_slow_down+1
    if key_set_slow_down==30:
        key_set_slow_down=0
        Key_Proc()
...
point_num=0
def find_black_rect():
    global loop,rect_solw,uart_flag,point_num
    sensor.reset()
    ...
    sensor.set_contrast(2)                              # 对比度
    sensor.set_saturation(0)                            # 饱和度
    sensor.set_gainceiling(128)                         # 增益上限
    sensor.set_auto_gain(True)                          # 增益
    sensor.set_auto_exposure(True)                      # 曝光速度
    ...
    while mod==2:
        ...
        str_1=0
        if uart_flag:
            if rect_solw>=0 and rect_solw<100 and point_num==0:
                str_0=(str)(get[2])
                str_1=(str)(get[3])
                #print('UART'+'*'+str_0+','+str_1+'#')
            if rect_solw>=100 and rect_solw<200 and point_num==1:
                str_0=(str)(get[0])
                str_1=(str)(get[1])
                uart_flag=0
                #print('UART'+'*'+str_0+','+str_1+'#')
            if rect_solw>=200 and rect_solw<300 and point_num==2:
                str_0=(str)(get[6])
                str_1=(str)(get[7])
                #print('UART'+'*'+str_0+','+str_1+'#')
            if rect_solw>=300 and rect_solw<400 and point_num==3:
                str_0=(str)(get[4])
                str_1=(str)(get[5])
            point_num=point_num+1
